document.addEventListener('DOMContentLoaded', () => {
    ...
    // View details – open modal with reservation info
    document.querySelectorAll('.view-details').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            fetch(barreAjax.ajaxurl, {
                ...
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: new URLSearchParams({
                    action: 'barre_get_reservation_details',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('detailsContent').innerHTML = 
                        '<p style="color:#dc3545;">Error: ' + (data.data?.message || 'Failed to load details') + '</p>';
                }
            })
            .catch(err => {
                document.getElementById('detailsContent').innerHTML = 
                    '<p style="color:#dc3545;">Connection error: ' + err.message + '</p>';
            });
        });
    });

    // Render the fetched details nicely
    function renderDetailsContent(details) {
        const content = document.getElementById('detailsContent');
        content.innerHTML = `
            <p><strong>Reservation ID:</strong> #${details.id}</p>
            <p><strong>Class:</strong> ${details.name}</p>
            <p><strong>Date & Time:</strong> ${details.date_formatted}</p>
            <p><strong>Duration:</strong> ${details.duration} minutes</p>
            <p><strong>Instructor:</strong> ${details.instructor}</p>
            <p><strong>Persons:</strong> ${details.num_persons}</p>
            <p><strong>Price per person:</strong> ${details.price_formatted} Kč</p>
            <p><strong>Total Price:</strong> ${details.total_formatted} Kč</p>
            <p><strong>Booked on:</strong> ${details.created_at_formatted}</p>
            ${details.cancelled_at_formatted ? 
                `<p><strong>Cancelled on:</strong> ${details.cancelled_at_formatted}</p>` : ''}
            ${details.rescheduled_at_formatted ? 
                `<p><strong>Last rescheduled:</strong> ${details.rescheduled_at_formatted}</p>` : ''}
        `;
    }

    // Close view details modal
    document.getElementById('closeViewDetails')?.addEventListener('click', closeViewDetailsModal);
